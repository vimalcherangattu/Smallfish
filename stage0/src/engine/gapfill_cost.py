...
- **Read cost** comes from the plan's cold/warm per-business figures.
- **Match rate** comes from the measured tallies in `public/data/index.json`.

Nothing here is estimated where something was measured. Where a figure is
still a planning estimate — the per-business read cost, until S0-15 meters a
real run — it is labelled as one in the output, because a cost model whose
inputs are half measured and half guessed is exactly the thing the gate exists
to prevent.
...
# Planning estimates from PROJECT_PLAN.md, not yet metered (S0-15).
...
def main() -> int:
    ...
    for r in base:
        ...
        read_per_match = COLD_READ_PER_BUSINESS / match_rate
    ...
    if priced:
        print("\nSENSITIVITY — the verdict above rests on an unmetered $%.3f read.\n"
              % COLD_READ_PER_BUSINESS)
        ...
        for mid, cov, mr, disc, _read, _total in priced:
            ...
            cold_total = COLD_READ_PER_BUSINESS / mr + disc
        ...
        print(
            "\n  Every market comes inside budget on warm reads, and none does on\n"
            "  cold ones. That is not a contradiction — it is the cold-market\n"
            "  problem the plan already names (Change 11), now with a price on it:\n"
            "  the first pass over a market loses money and the second does not."
        )

    print(
        "\nWhat is measured and what is not:\n"
        "  measured  — discovery cost per business, coverage, match rate\n"
        "  estimated — $%.3f cold read per business, until S0-15 meters a real run.\n"
        "              Read cost dominates the total, so this estimate is the one\n"
        "              that matters. Gate item 4 is the real test, not this model."
        % COLD_READ_PER_BUSINESS
    )
