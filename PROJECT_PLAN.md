...
- [x] **S1-22 · ICP discovery — read the seller's own site.** `src/lib/seller.ts` +
  `/api/read-seller`. Paste a URL, get what they sell, who they serve, their problem
  language and their geography — **each with the sentence it came from**, and a field
  whose quote is not in the fetched text is dropped rather than returned. As promised,
  it upgrades the *input* and changes nothing downstream: the profile becomes the same
  free-text description `inferFromOffer` already consumed.
  *Deliberately not the Python crawler.* `site_probe.py` exists to fetch thousands of
  sites whose owners did not ask; this fetches one site because its owner pasted the URL
  and pressed a button, so the throttle and cache have nothing to be polite across.
  robots.txt is still honoured, and fails **open** — a robots.txt we could not read is
  not a disallow, and treating it as one refuses an owner their own site.
  *Still to do:* share the implementation with GTM automation A2, which needs the same
  capability aimed at prospects rather than at the customer.
...
- [x] **S1-05b · Keep link targets in the fetcher**, so socials and mailto-only addresses
  ...
  **Measured on a fresh sample, 25 sites read of 45 tried: socials go from 4% to 68%
  of sites, a 17× recovery.** That is what was hiding in attributes. Emails gained
  **nothing** — every `mailto:` address was also printed in visible text, and
  `emails_recovered_from_mailto` is 0.
...
- [x] **S1-08 · Accounts, billing and plans** (Free, Starter, Growth) plus packs, on Stripe.
  Clerk for identity, Supabase Postgres for the ledger, Stripe for the card. Sign-up,
  workspace creation, the free grant, checkout, the paid grant, monthly renewal and
  cancellation all run against live credentials. **The account page creates a workspace
  when it finds none**, so a webhook is an optimisation rather than a dependency — see
  the decision log for the signup that proved why. `apply_paid_period` claims Stripe's
  event id with a primary key in the same transaction that grants the credits, so a
  redelivered `invoice.paid` cannot mint a month nobody paid for.
  *Not built:* dunning. `invoice.payment_failed` is deliberately unhandled rather than
  stubbed, because how many retries, what the customer sees and when access stops are
  real decisions and a stub would make them look made.
...
| 2026-09-24 | **Supabase Postgres and Supabase Auth, not Clerk — and the reason is RLS, not cost** | The ledger has to live in Postgres whatever happens: `unlocks` needs a primary key on `(account_id, business_id)` or two concurrent requests both pass `ledger.ts`'s already-unlocked check and both charge, and a balance that can be `UPDATE`d is a history that is a suggestion. Given Postgres, the question is only where identity comes from. Supabase Auth puts `auth.uid()` directly inside the row-level policies, so "one workspace cannot read another's balance" is a database fact rather than a thing every query has to remember. Clerk can drive the same policies through a JWT, but that is a joint to maintain, and a misconfigured joint fails in one of two ways — deny everything, which is visible, or allow everything, which is not. **The honest case for Clerk is S2-03:** agency workspaces with invitations are Clerk Organizations almost exactly, and rebuilding that is real work. It is two gates away and blocked on Stripe, and buying a second identity vendor now to save work later is how a product at zero users ends up with two user tables. Revisit at S2-03, when the requirement is concrete. The migration names the exact change if it flips: `account_members.user_id` becomes text and every `auth.uid()` becomes a claim lookup. |
| 2026-09-24 | **Pricing stays in TypeScript; only atomicity moves to SQL** | The tempting shape is a `charge_for_match` that knows about bands. It would be a second source of truth about money, and the two would diverge on the first pricing change. The split: **TypeScript decides how much, Postgres decides whether it is allowed.** Band settlement, the quote-can-only-go-down rule and the 12-month unlock window stay in `pricing.ts` and `ledger.ts`, where 86 assertions run with no database and no network. The SQL function takes an already-settled cost and enforces the two things that genuinely cannot be decided outside a transaction: sufficient balance under a row lock, and not already unlocked. `test_schema.py` fails if the words `band`, `settle` or `wilson` appear in the schema's code. |
| 2026-09-24 | **The opt-out promise was being kept by hand, and nobody had said so** | `S1-09` promises removal from future searches within seven days, and the app read the suppression list from `public/data/suppressed.json` — a file in the repository. So honouring a removal required a commit and a deploy: a person remembering to do something, for a promise made to people who are not customers and never agreed to be in the product. The list now comes from `/api/suppressed`, which reads the database when one is configured and serves the same committed file when none is, saying which. Three things the route does deliberately: it returns the **union** of both sources, because a business in one and not the other should be suppressed rather than adjudicated; it serves only rows whose `effective_at` is set, because applying a removal before it has been processed is as wrong as applying it late; and **a database read failure serves the file and returns 200**, not a 5xx, because the payload is still the right one to apply and a client that discards it shows every business that asked to be left out. |
| 2026-09-25 | **Login is Clerk. The previous day's recommendation is overruled, and the cost is named rather than minimised** | Yesterday's entry recommended Supabase Auth on the grounds that `auth.uid()` inside the policies makes a cross-workspace read structurally impossible. The decision is Clerk, and the reasoning against it does not disappear because the decision went the other way — it becomes the thing to watch. What is actually lost: `account_members.user_id` can no longer be a foreign key into `auth.users`, so **nothing in the database guarantees a member row points at a person who exists**. That check moves to the `user.created` webhook, which is a weaker place for it, and it is a price rather than an oversight. What replaces `auth.uid()` is `auth.jwt() ->> 'sub'`, populated only once Clerk is registered as a third-party auth provider in the Supabase dashboard; until it is, `member_of` returns false and every policy denies. That is the right direction to fail in and it is deliberate: a policy that passes when the claim is missing is the failure with no symptom. What Clerk buys, and it is real: S2-03's agency workspaces with invitations are Clerk Organizations almost exactly, so the work that entry deferred does not have to be built. `0001` had named this exact migration in advance, which is why it was four statements. |
| 2026-09-25 | **The schema shipped two bugs that no amount of reading it could find, and both were caught in the first ten minutes against real Postgres** | The migration was committed with a lint over it — `test_schema.py`, 27 checks — and it read perfectly. Applied to a database, it failed twice. **First:** `charge_for_match` declares `milli` as an OUT parameter *and* sums a column called `milli`, so `sum(milli)` is ambiguous and the function raises `42702` on the very first charge — not on an edge case, on the happy path. **Second:** `ledger_entries.account_id` carried `ON DELETE CASCADE` while a trigger on the same table refuses every `DELETE`, so deleting an account raised `P0001` from inside the cascade and the cascade could never have fired. The schema documented a behaviour it did not have. Corrected to `ON DELETE RESTRICT`, which states the truth: an account with billing history is **closed**, not deleted, and `accounts.closed_at` is what that column was always for. Eleven cases now verified against project `xdqptokitwdhfuotfpph` in a transaction that rolls back: a charge that fits; the same business refused by the primary key on `unlocks`; a charge over the balance refused; `UPDATE` and `DELETE` on the ledger both refused by the trigger; `DELETE` on the account refused by the new constraint; an event carrying a business name refused by the check constraint; an event carrying only counts accepted; the balance arriving at exactly 3000 milli; closing the account working; and `member_of` returning false with no JWT. This is the fourth time in this project that running something has corrected a document about it, and the lint's docstring now says what a lint is and is not for. |
| 2026-09-25 | **The obvious version of the opt-out form would have been a tool for erasing a competitor, which is what the page it sits on warns against** | The page has said since it was written that "an opt-out anyone could file for anyone else would be a tool for erasing a competitor". The natural form — type your domain, we remove you — **is that tool**, and it took writing it to see why: the listed domain and phone are *public*. They are printed on the business's own website, which is where we got them. So `verifies()` matching a claim against the listing is a **filter, not a proof**; it stops a request from someone who does not even know the business's details, and that is all it does. The form therefore files a row with `remove_by` set and `effective_at` null, which the public read policy does not serve, and confirmation goes to the contact **already on the listing** rather than the one typed. Verified against the database: anon sees the confirmed row and not the pending one, and cannot insert a removal at all. The honest cost is that **confirmation is manual until a mailer exists**, named in the launch checklist rather than closed by letting the form take effect on its own. Matching is on the contact rather than a business id for a second reason the billing work already established: 23 Phoenix listings share `aspendental.com`, so a removal keyed on the one id somebody happened to find would leave 22 practices listed. |
| 2026-09-25 | **Three of this repo's own tests have now fired on the comment explaining why the thing they looked for is absent** | `test_schema.py` failed on "no floating-point column" because a migration comment used the word *real*; it failed again on "the schema does not restate pricing" because a comment explained why bands are not in SQL; and `test_optout.mjs` failed on "the route never sets `effective_at`" because the route's docstring says it deliberately never writes it. Each is the same mistake: a check about what code **does**, run against prose. Both files now strip comments before any code-level check, and `test_optout.mjs` additionally joins string concatenations and collapses whitespace, because a fourth failure was the test matching across a `" +` line wrap rather than the sentence a user reads. Worth recording because the failure mode is seductive: these checks pass on a codebase with no comments, which is the one where they are least useful. |
| 2026-09-25 | **The read allowance — the only bound on a criterion that matches nothing — shipped failing open, and running it is what showed that** | `spend_reads` returned the number of reads *asked for* rather than the number permitted. An account already at its allowance asked for 30 more and was told it could have them. That is the cost guard inverted, and it is the guard that matters most: credits deplete on matches, so a criterion nothing satisfies never touches the balance, and the read allowance is the only thing between an impossible search and an unbounded bill. Everything else in the pricing design assumes it holds. The cause was arithmetic inside a `RETURNING` clause, where `reads_this_period` is the value *after* the update, so subtracting the requested figure cannot recover the value before it once clamping has happened — the clamp was real in the database and invisible to the caller. Computed before the update now, under the same row lock the other ledger writes take. Verified: 30 of 44, then 14, then 0, counter never above 44, a negative request returning 0 and moving nothing, an unknown account returning 0 rather than the requested figure. `test_schema.py` fails if the arithmetic moves back inside a `RETURNING`. |
| 2026-09-25 | **A second expectation in that run was wrong, and the code was right — recorded because the reverse is the usual story here** | The verification expected an account's balance to be capped at one period's allowance after renewal and read 8000 against an expected 4000. The code is correct: rollover is capped at one allowance and *then* the new period is granted, so a full period ends at twice the allowance — one month carried plus this month's grant. `renew()` in `ledger.ts` has always done exactly this and `test_ledger.mjs` asserts it. The test expectation was the error. Noted because four of the last five things verified against real infrastructure turned out to be defects in the thing being verified, and treating that as the default would have "fixed" a correct rollover rule into a wrong one. |
| 2026-09-25 | **The first real signup produced no account, because a webhook was the only way one could exist** | `user.created` was the single path by which a workspace came into being. The signup landed during the Cloudflare cutover, while `www` was proxied without a certificate: Clerk's delivery failed at the TLS handshake exactly as a browser did, and the customer arrived signed in, account-less, looking at an explanation they had no way to act on. The DNS was the proximate cause and is fixable in thirty seconds, but the **shape** was the real defect. "At least once" delivery is a guarantee about a webhook that can be delivered at all; a misconfigured endpoint, an expired signing secret or a broken certificate all mean zero, and each of those is likelier than it sounds in the first week of a product. The account page now creates the workspace when it finds none, so the webhook is an optimisation — it gets there before the first page load — rather than a dependency. `ensureWorkspace` checks for an existing membership and the `(account_id, user_id)` primary key sits underneath it, so the two racing cannot mint two free grants. A write during a page render is unusual and deliberate: the alternative is showing someone an error about their own missing account. |
| 2026-09-25 | **The first workspace was created and never funded, and it looked fine** | `ensureWorkspace` creates an account row, a membership row, then the period's credit grant. The membership insert sends `Prefer: return=minimal`, to which PostgREST replies **201 Created with an empty body** — and the HTTP helper guarded against 204 alone, so `res.json()` threw on an empty string and the function died between step two and step three. What the customer saw was the worst available version of that: not an error, but an account with a member, a plan, and a read allowance of 220 printed on screen, holding **zero credits**. A funded-looking account with no funds. The database settled it in one field — `period_start` was equal to `created_at` to the microsecond, and `renew_period` sets `period_start = now()`, so it had plainly never run. Two lessons kept as code: **a status code is not a reliable signal that a body exists**, so the helper now reads the text and parses only if there is something to parse; and every list reader tolerates an empty body rather than indexing into nothing. `test_accounts_http.mjs` drives the real module against a stubbed `fetch` that answers exactly as PostgREST does, including the empty 201, and fails if the grant is not reached or if a second call to a funded account grants a second period. A first guess at the cause — that the `revoke all on function` in `0003` had stripped `service_role`'s EXECUTE — was checked against `has_function_privilege` and was wrong; recorded because checking took thirty seconds and would otherwise have produced a confident fix to a working thing. |
| 2026-09-25 | **The payment path refuses in four places rather than guessing, and each refusal is a way of being quietly wrong** | Writing the Stripe half made the failure modes concrete, and they are asymmetric: missing an event leaves a paying customer with no credits, while applying one twice mints a month nobody paid for into a ledger that cannot be corrected by deletion. **Whose payment it is** comes from `client_reference_id` or subscription metadata and never from an email address — a customer can pay with a different address from the one they signed up with, and matching on it credits someone else's workspace, which is worse than crediting none because the payer still has nothing and the error is silent. **Which plan** comes from the price id, not the amount paid, because two plans could one day cost the same. **Who may start a checkout** comes from the signed-in session, never the request body, or any caller could attach a card to another workspace. And **the raw body** is what the signature is verified against: parsing to JSON and re-serialising changes the bytes and fails every signature, so the handler never calls `request.json()`. A missing credential returns 500 rather than 200, because a 200 tells Stripe the payment was handled and it stops retrying. |
| 2026-09-25 | **A test that asserted the checkout was unwritten is replaced rather than deleted** | `test_s1_platform.mjs` asserted that `startCheckout` refused *even with credentials present*, which was correct while the session call was deliberately unwritten. That assertion is now false, and the tempting move is to delete it. But it was never protecting "refuse always" — it was protecting **never return a session url that did not come from Stripe**, because a checkout that silently no-ops in development is how billing ships untested. So it now drives the real function with plausible-looking fake keys and requires that it fails against Stripe and passes Stripe's own message through. The property survives its original implementation, which is the point of writing down what a check is for. |
| 2026-09-25 | **A ₹2 test price exposed that the pricing page's dollars may not be what Stripe charges** | The verification box read `VERIFICATION_PLAN.priceUsd` and told the customer "a real $1 charge". The Stripe price was ₹2. The field was never connected to the Stripe price object and never could be — the amount and its currency live in Stripe, and nothing in this repository knows them. The screen now states no amount at all: Stripe shows it at checkout, where it is true. `priceUsd` stays at 1 as a **gate** rather than a price, because `startCheckout` refuses any plan whose `priceUsd` is 0 and that is the free-plan rule. **The open question this raises is larger than the test price.** `docs/PRICING.md`, the pricing page, the terms and every solvency calculation are denominated in **USD** — $29, $79, $199, a $0.0168 cost per read, a $3.36 worst-case loss on one scan. If the Stripe price objects are denominated in INR because the account settles in INR, a customer sees a different number at checkout from the one the page promised, and the band arithmetic is being done in a currency the business does not receive. To check before anyone is charged for a real plan: the currency of `STRIPE_PRICE_STARTER`, `_GROWTH` and `_AGENCY`. |
| 2026-09-25 | **The first live payment granted two periods, and the event id was the wrong key** | A ₹2 verification charge produced `checkout.session.completed` and `invoice.paid`, both describing the same new subscription, each carrying its own Stripe event id. The primary key added in `0004` saw two distinct events and let both through, so one purchase granted two periods — 1 extra credit here, **400 on Growth**, into an append-only ledger that cannot be corrected by deleting a row. The event id makes a *redelivery* idempotent, which is necessary and is not sufficient: what has to be unique is the **period being paid for**, however many events mention it. `0005` adds `period_key`, a second unique index on `<subscription>:<period>` — `create` for a subscription's first period so the session and its first invoice collide, and the period start for every cycle after it so each renewal grants exactly once. Keying renewals on `create` too would have been the opposite failure and a worse one: every monthly renewal refused, a customer paying and receiving nothing. The old eight-argument signature is dropped rather than left callable, because a call that omits the period key is a call that can double-grant. **This was found by spending ₹2 rather than by reading the code**, which is the fourth time in this project that running something has corrected a document about it. |
| 2026-09-25 | **A text box that makes the server fetch a URL is a server-side request forgery hole, and the first version had one** | `safeUrl` prefixed anything not starting with `http` with `https://`, as a convenience so a customer could type `example.com`. That convenience turned `file:///etc/passwd` into `https://file:///etc/passwd` — which parses, whose protocol is then `https:`, and which sailed straight past the protocol check underneath it. The same laundering worked for `ftp://`. A scheme that is present is now required to be http(s) *before* any prefixing, so the convenience cannot launder one. Caught by the test rather than by review, which is the argument for writing the adversarial cases down: the blocklist for `localhost`, private ranges and `169.254.169.254` was already there and was never reached. |
| 2026-09-25 | **A fifth test failed on how a character was spelled rather than on what the code does** | The check for quote normalisation grepped for a `‘` escape; the file uses the literal curly apostrophe, so it failed against correct code. `flat` is now exported and the test exercises it — `flat("don't")` against `flat("don’t")`. This is the same mistake as the four comment-greps, in a new costume: a source-level check asserting the author's typing. The rule that survives is the one in `_source.py` — a check about what code *does* has to run the code. |
